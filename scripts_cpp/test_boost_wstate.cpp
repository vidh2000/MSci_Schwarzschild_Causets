...
double BH_int_dvarphi_du(double u1, double u2, double M, double c2)
{
    ...
    auto BH_dvarphi_du_forint = [M, c2]
                            (state& varphi, state& dpdu, double u)
                            {dpdu = std::pow(2*M*u*u*u - u*u + c2, -0.5);};
    state varphi = 0;
    if (u2>=u1)
    {
        size_t smth = boost::numeric::odeint::integrate(BH_dvarphi_du_forint, varphi, 
                                          u1, u2, (u2-u1)/20.);
    }
    else
    {
...
double BH_int_dt_du (double u1, double u2, double M, double c)
{
    ...
    if (u2>=u1)
    {
        auto BH_dt_du_forint_plus = [M, c]
                              (const state& t, state& dtdu, const double u)
...
bool BH_time_caus_check(double u1, double u2, double t2, double M, double c)
